...
Initialize dfa . num_alphabets = nfa . num _alphabets , i = 0
Intialize a set lazySet which stores subsets of Q and store {0}
in it .
Create a new row of size dfa . num_alphabets and insert into dfa .
table and initialize all values to -1.
While i < lazySet . size ()
Iterate through each of the input symbol j in E
Initialize an empty set of states reachable and
a variable next = -1
Iterate i through each element in lazySet [ i ] and push
into reachable the set nfa . table [ i ][ j ]
Check if reachable is already in lazySet . If yes ,
the get the value of next from ‘ lazySet . If not , then
Insert into lazySet , the set reachable and
set next = lazySet . size ()
Insert next into dfa . finalStates if any element
in reachable is a final state of the original nfa
Create a new row of size dfa . num_alphabets and
insert into dfa . table and
